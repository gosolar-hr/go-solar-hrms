...
const nav = [
  ...
  {
    href: '/my-comp-off', label: 'My Comp Off', hrOnly: false, techOnly: true,
    icon: <svg className="nav-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>,
  },
  {
...
export default function Layout({ children }) {
  ...
  const visibleNav = nav.filter(item => {
    if (role === 'tech') return !item.hrOnly  // tech sees non-hrOnly items (including techOnly)
    return !item.techOnly                      // HR sees everything except techOnly
  })
