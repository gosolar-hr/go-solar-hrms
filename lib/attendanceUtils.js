...
import { getWeekOffDates } from './weekoffs'

function pad(n) {
  return String(n).padStart(2, '0')
}

export async function ensureMonthlyAttendanceDetails(employee, month, year, holidayDates = null) {
  const employee_id = employee?.id
  if (!employee_id) return { inserted: 0 }
  ...
  // Employee had not joined yet in this payroll month.
  if (joiningDate && joiningDate > monthEnd) {
    return { inserted: 0 }
  }

  const firstPayableDay = joiningDate && joiningDate > monthStart
    ? joiningDate.getDate()
    : 1

  const from = `${year}-${pad(month)}-${pad(firstPayableDay)}`
  const to = `${year}-${pad(month)}-${daysInMonth}`
  ...
  for (let day = firstPayableDay; day <= daysInMonth; day++) {
    const dateStr = `${year}-${pad(month)}-${pad(day)}`
    if (!existingDates.has(dateStr)) missingDates.push(dateStr)
  }

  if (missingDates.length === 0) return { inserted: 0 }

  const { weekOffDates, holidayDates: finalHolidayDates } = await getWeekOffDates(
    year,
    month,
  ...
  const rows = missingDates.map(dateStr => {
    ...
    if (finalHolidayDates.has(dateStr)) {
      status = 'H'
    } else if (weekOffDates.has(dateStr)) {
      status = 'W/O'
    }
...
export async function refreshAttendanceSummary(employee_id, month, year) {
  ...
  for (const d of allDays || []) {
    ...
    else if (s === 'PL' || s === 'CO')                       { present_days++ }
