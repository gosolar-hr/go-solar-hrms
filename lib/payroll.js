...
export function calculatePF(employee) {
  ...
  const pfWage = Number(employee.basic_salary || 0) +
...
export function calculateESIC(employee, earnedGross, earnedCTC = 0) {
  ...
  // FIX: Prorate conveyance using earnedGross as the base (same as what
  // we subtract from), NOT earnedCTC. Previously using earnedCTC here
  // caused the conveyance deduction to be understated for OT employees,
  // meaning OT was not correctly reflected in the ESIC wage.
  let earnedConveyance = 0
  if (totalCTC > 0) {
    earnedConveyance = (Number(employee.conveyance || 0) / totalCTC) * earnedGross
  }
