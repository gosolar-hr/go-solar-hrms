...
export function calculateGrossSalary(
  ...
  if (payrollMonth && payrollYear && date_of_joining) {
    ...
    if (joiningYear === payrollYear && joiningMonth === payrollMonth) {
      // Employee joined this month — calculate working days from joining date
  ...
  // Use actual present days but cap at effective days
  const workingPresent = Math.min(
    attendance.present_days,
    effectiveDays
  )

  const earnedSalary  = perDaySalary * workingPresent
  ...
  return {
    ...
    effectiveDays,   // ← useful for payslip display
  }
}
