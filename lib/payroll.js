...
// ─────────────────────────────────────────────────────────
export function calculateESIC(employee, earnedGross, overtimeAmount = 0) {
  const conveyance = Number(employee.conveyance || 0)

  // Contracted gross = full monthly CTC
  const contractedGross = Number(employee.basic_salary  || 0) +
                          Number(employee.hra            || 0) +
                          Number(employee.cca            || 0) +
                          Number(employee.conveyance     || 0) +
                          Number(employee.allowances     || 0)

  // Applicability check uses CONTRACTED gross
  if (contractedGross > 21000) {
    return { employee: 0, employer: 0, esicWage: 0, applicable: false }
  }

  // ESIC wage = earned gross - conveyance + overtime (floor at 0)
  const esicWage = Math.max(0, earnedGross - conveyance + overtimeAmount)
