...
export function calculateGrossSalary(
  ...
  const gross = Math.max(0, earnedCTC) + overtimeAmount + incentive
...
// STEP 3: PF
// Per EPF Act:
//   PF wage = Basic + CCA + Allowances (HRA and Conveyance excluded)
//   LWP does not reduce the PF contribution wage base — it reduces gross take-home.
// ─────────────────────────────────────────────────────────
export function calculatePF(employee, earnedBasic) {
  if (!employee.pf_applicable) return 0

  const masterBasic = Number(employee.basic_salary || 0)
  const earned = Number(earnedBasic || 0)

  if (earned <= 0) return 0

  // For employees below PF wage ceiling, PF follows earned basic.
  if (masterBasic < 15000) {
    return Math.round(earned * 0.12)
  }

  // For PF ceiling employees, keep full capped PF unless earned basic is very low.
  if (earned < 12000) {
    return Math.round(earned * 0.12)
  }
...
export function calculateESIC(employee, earnedGross, earnedCTC = 0) {
  ...
  // Eligibility check against FIXED monthly wage (no OT, no proration)
  ...
  // ESIC Act mandates rounding up to the nearest rupee
  return {
    employee   : Math.ceil(esicWage * 0.0075),
    employer   : Math.ceil(esicWage * 0.0325),
...
export function calculateNetSalary({
  ...
  loan = 0, advance = 0, otherDeduction = 0
}) {
  const totalDeductions =
    pf + esicEmployee + pt + loan + advance + otherDeduction

  return Math.round(Math.max(0, gross - totalDeductions))
}
// ─────────────────────────────────────────────────────────────
// OTHER DEDUCTIONS
